...
Here, we form a new instance of the parent constructor for every instantiation of the child constructor. So we would'nt be having the problem of mulitple references to instance variables of the parent class like in the previous example.  
...
Using object oriented principles with javascript *may* seem straigtforward, but can get a little tricky when used with the browsers DOM API. Do check out my [other post](/blog/2016/05/06/using-oops-in-browser/) on using these patterns along with the browsers API for some real world examples on using OOP with javascript.
